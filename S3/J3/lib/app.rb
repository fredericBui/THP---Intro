...
def hello_world(client)
    ...
    #Improvement with number.times do
    (1..5).each do
        journalist = journalists[rand(journalists.length())]
        client.update("J'ai adoré votre dernier article ! #{journalist} #bonjour_monde")
    end
end

def like_hashtag(client,hashtag)
    client.search("#{hashtag} -rt", lang: "fr").take(5).collect do |tweet|
        puts "#{tweet.user.screen_name}: #{tweet.text}"
        #tweet.like
    end
end

like_hashtag(login_twitter,"bonjour_monde")
